...
class SAM3DObjectsEncoder(nn.Module):
    ...
    def forward(self, x, object_mask=None, output_size=None, object_prompt=None):
        ...
        ``x`` is DECO's normalized RGB crop and ``object_prompt`` is the
        human-removed union of Task-6 per-keypoint-circle SAM masks. The dataset
        applies the same crop/resize to both; this method then recreates SAM-3D-Objects'
