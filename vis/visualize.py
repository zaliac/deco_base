...
def create_scene(mesh, img, focal_length=500, camera_center=250, img_res=500):
    ...
    # add body mesh. NOTE: do NOT pass a material to Mesh.from_trimesh below.
    # pyrender only uses the mesh's per-vertex colors (which encode contact) when
    # material is None (see pyrender _get_trimesh_props); passing a material makes
    # it render a uniform color and silently discards the contact colors.
    mesh_images = []
    ...
    for sideview_angle in [0, 90, 180, 270]:
        ...
        #     out_mesh,
        #     material=material)
        out_mesh = pyrender.Mesh.from_trimesh(out_mesh)
        mesh_pose = np.eye(4)
...
def gen_render(output, normalize=True):
    ...
    base_color = np.array([200, 200, 200, 255], dtype=np.uint8)   # grey body
    contact_color = np.array([255, 0, 0, 255], dtype=np.uint8)    # contact vertices in red

    n_verts = len(gt_mesh.vertices)

    gt_vertex_colors = np.tile(base_color, (n_verts, 1))
    gt_vertex_colors[contact_labels_3d[0, :] >= th] = contact_color
    gt_mesh.visual.vertex_colors = gt_vertex_colors

    pred_vertex_colors = np.tile(base_color, (n_verts, 1))
    pred_vertex_colors[contact_labels_3d_pred[0, :] >= th] = contact_color
    pred_mesh.visual.vertex_colors = pred_vertex_colors
